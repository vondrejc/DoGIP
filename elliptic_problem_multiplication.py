...
B0 = get_B0(dim, pol_order, problem=1) # interpolation between V on W on a reference element
AT_dogip = get_A_T(A, V, W, problem=1)
...
def system_multiplication_DoGIP(AT_dogip, B, u_vec):
    ...
    for ii, cell in enumerate(cells(mesh)):
        ...
        Au[ind] += np.einsum('rjl,rsj,sjk,k->l', B, AT_dogip[ii], B, u_vec[ind])
    return Au

u, v = TrialFunction(V), TestFunction(V)
Asp = assemble(A*inner(grad(u),grad(v))*dx, tensor=EigenMatrix())
Asp = Asp.sparray()
