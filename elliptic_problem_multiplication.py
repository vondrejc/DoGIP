...
from fenics import (UnitSquareMesh, UnitCubeMesh, FunctionSpace, Function, TestFunction,
                    TrialFunction, Expression, assemble, dx, cells, inner, grad, EigenMatrix)
import numpy as np
...
B0 = get_Bhat(dim, pol_order, problem=1) # interpolation between V on W on a reference element
...
def system_multiplication_DoGIP(AT_dogip, B, u_vec):
    ...
    for ii, cell in enumerate(cells(mesh)):
        ...
        Au[ind] += np.einsum('rjl,rj->l', B, ABu)
    return Au

u, v = TrialFunction(V), TestFunction(V)
Asp = assemble(m*inner(grad(u),grad(v))*dx, tensor=EigenMatrix()) # system matrix of FEM
Asp = Asp.sparray() # matrix-vector multiplication in FEM

Au_DoGIP = system_multiplication_DoGIP(AT_dogip, B0, ur_vec) # matrix-vector mul. in DoGIP
